...
print(width,height)
r1,g1,b1,a1,front,back,total,lengthcounter,lengthcounter2=0,0,0,0,0,0,0,0,0#基础数据
pictureLength=100#图片总长度（厘米数
precise=1#精度
wordsize=25#调节字体大小
k=pictureLength/height
lr,lg,lb,la=[0],[0],[0],[0]#基础数据
lpixel,ll,lc,lPrintPosition=[],[],[],[]#基础数据
alphabat=['A','A','B','C','D','E','F','G','H','I','J','K','L','M','N']#以ABCDEFG标注颜色
lpantone=["default",'xx-xxxx TPX','xx-xxxx TPX','xx-xxxx TPX','xx-xxxx TPX','xx-xxxx TPX','xx-xxxx TPX','xx-xxxx TPX'] #色卡序号，default和xx不打印，用于检查是否出错


for i in range(0,height):
    r2,g2,b2,a2=image.getpixel((width*9/10,i))
    if (r2-precise<=r1<=r2+precise and g2-precise<=g1<=g2+precise and b2-precise<=b1<=b2+precise and a2-precise<=a1<=a2+precise):
        thingtodo=0
    else:
        front=i
        r,g,b,a=image.getpixel((width/2,i))
        r1=r2
        g1=g2
        b1=b2
        a1=a2
        ...
        for j in range(0,len(lr)):
            if(lr[j]-precise<=r<=lr[j]+precise and lg[j]-precise<=g<=lg[j]+precise and lb[j]-precise<=b<=lb[j]+precise and a2-precise<=a1<=a2+precise) :
                lc.append(j)
                break
            elif j==len(lr)-1:
                lr.append(r)
...
draw.text((truewidth-400,10),u'100mm*100mm',(0,0,0),Fontwords)#改变打印出来的厘米数
...
for i in range(1,len(lr)):
    draw3.rectangle((finalWidth-1400,yposition,finalWidth-1000,yposition+90),(lr[i],lg[i],lb[i],la[i]),'black')#300/500
    #print(lr[i],lg[i],lb[i])#300/500#需要知道RGB值时使用
