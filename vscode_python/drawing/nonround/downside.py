...
print("Hello, world!")
image = Image.open("C:\\Users\\userHu\\Pictures\\Saved Pictures\\test2_4.png")#条纹图
image=image.convert("RGB")
image.show()
truewidth2=2000#2000/1200
trueheight2=2000#2000/1200
truewidth=1800#1920/1080
trueheight=1800#1920/1080
...
lpantone=["default",'14-3905 TCX','11-0103 TCX','17-3925 TCX','18-3834 TCX','13-1025 TCX','12-1706 TCX','xx-xxxxTPX'] #色卡序号，default和xx不打印，用于检查是否出错
...
ll.append((height-front)*k)
lpixel.append((height-front)*rate)#去除空值并添加新值
...
imageOrignal = Image.open("C:\\Users\\userHu\\Pictures\\Saved Pictures\\test1_4.png")#蒙版图
...
#for i in range(0,len(lc)-1):
    #if alphabat[lc[i]]=='I' or alphabat[lc[i]]=='K':
        #alphabat[lc[i]]='F'

for i in range(0,len(lPrintPosition)-1):
    if(lPrintPosition[i+1]-lPrintPosition[i])<=wordsize*0.8 or (lPrintPosition[i]-lPrintPosition[i-1])<=wordsize*0.8:#1.4*wordsize
        linescounter+=1
        ...
        #if alphabat[lc[i]]=='G' or alphabat[lc[i]]=='H':
            #alphabat[lc[i]]='C'
        draw.text((lengthreminder*linescounter+5+2*wordsize+0,lPrintPosition[i]+(trueheight2-trueheight)/2-wordsize),str(alphabat[lc[i]]),(255,69,0),Fontspecial)
    else:
        linescounter=0
        ...
        #if alphabat[lc[i]]=='G' or alphabat[lc[i]]=='H':
            #alphabat[lc[i]]='C'
        draw.text((lengthreminder+5+2*wordsize+0,lPrintPosition[i]+(trueheight2-trueheight)/2-wordsize),str(alphabat[lc[i]]),(255,69,0),Fontspecial)
...
draw.text((truewidth-500,10),u'90mm*90mm',(0,0,0),Fontwords)#改变打印出来的厘米数
